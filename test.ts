...
import {Coords, DummyMap, MapFactory, PointData, PointTemplate} from './index';
...
describe("map", function () {
  ...
  it('should init map', function () {
    ...
    let map = MapFactory.createComp(DummyMap, elem('map'));
    expect(map).to.not.be.null;
    expect(map.root).to.be.equal(elem('map'));
    expect(MapFactory.fromRoot(elem('map'))).to.be.equal(map);
  ...
  it('should init map using functor', function (done) {
    ...
    MapFactory.initByFunc((root, options) => {
